#!/usr/bin/env python3
"""
Advanced Multi-Camera Face Recognition System
Supports single or multiple cameras (USB, webcam, RTSP streams)
Comment/uncomment cameras in camera_config as needed
"""

import cv2
import numpy as np
import json
import os
import time
from datetime import datetime
import threading
import webbrowser
from http.server import HTTPServer, SimpleHTTPRequestHandler
import signal
import sys
import base64

# Import our modular face recognition models
try:
    import torch
    from face_models import ModelManager
    FACE_MODELS_AVAILABLE = True
except ImportError:
    print("⚠️ face_models.py not found or PyTorch not available. Using basic face detection only.")
    FACE_MODELS_AVAILABLE = False
    torch = None

class StableFaceTracker:
    def __init__(self):
        # ========== CAMERA CONFIGURATION ==========
        # Comment/uncomment cameras as needed
        self.camera_config = {
            "CAMERA_0": "0",   
            "CAMERA_1": "1",  
            "CAMERA_RTSP_101": "rtsp://Koy%20Otaniemen%20T:Otaranta123@10.19.55.20:554/Streaming/Channels/101",
            "CAMERA_RTSP_601": "rtsp://Koy%20Otaniemen%20T:Otaranta123@10.19.55.20:554/Streaming/Channels/601",
        }
        # ==========================================
        
        # Use optimized cascade for detection
        cascade_path = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
        self.face_cascade = cv2.CascadeClassifier(cascade_path)
        if self.face_cascade.empty():
            print(f"❌ Failed to load face cascade from {cascade_path}")
        else:
            print(f"✅ Loaded face cascade from {cascade_path}")
        
        # Initialize face recognition models
        if FACE_MODELS_AVAILABLE:
            device = 'cuda' if torch.cuda.is_available() else 'cpu'
            self.model_manager = ModelManager(device)
            self.current_model = 'arcface'  # Default model
            self.use_embeddings = True  # Start with face recognition enabled
            print(f"🤖 Face recognition models available on {device}")
            # Load the default model on startup
            if self.model_manager.load_model(self.current_model):
                print(f"✅ Loaded default model: {self.current_model}")
            else:
                print(f"⚠️ Failed to load {self.current_model}, falling back to basic tracking")
                self.use_embeddings = False
        else:
            self.model_manager = None
            self.use_embeddings = False
            print("📊 Using basic face tracking only")
        
        # Global tracking parameters
        self.global_tracks = {}  # Global face tracks across all cameras
        self.next_id = 1
        self.max_disappeared = 30  # Frames before considering face lost
        self.max_distance = 150  # Max distance for face matching
        self.min_stability_for_ui = 10  # Threshold for UI updates
        self.embedding_threshold = 0.6  # Cosine distance threshold for face matching
        
        # Auto-registration parameters
        self.auto_register_threshold = 25  # Auto-register when stability reaches this level
        self.auto_registered_faces = set()  # Track which faces we've auto-registered
        
        # Database
        self.faces_db = {}
        self.load_database()
        
        # Camera management
        self.camera_feeds = {}
        self.camera_threads = {}
        self.camera_frames = {}
        self.camera_fps = {}
        self.running = False
        
        # Stable faces for UI (only update when very stable)
        self.stable_faces = {}
        self.last_ui_update = 0
        
        # Determine if single or multi-camera mode
        self.is_multi_camera = len(self.camera_config) > 1
        
        os.makedirs('faces', exist_ok=True)
        print(f"Face tracker initialized. Database: {len(self.faces_db)} faces")
        print(f"Camera mode: {'Multi-camera' if self.is_multi_camera else 'Single camera'} ({len(self.camera_config)} cameras configured)")
        print(f"Auto-registration enabled: faces will be saved automatically with temp IDs")
        
    def detect_faces(self, frame, camera_id=None):
        """Optimized face detection"""
        if frame is None:
            print(f"⚠️ Frame is None for {camera_id}")
            return []
            
        # Resize frame for faster processing
        scale_factor = 0.75
        small_frame = cv2.resize(frame, None, fx=scale_factor, fy=scale_factor)
        gray = cv2.cvtColor(small_frame, cv2.COLOR_BGR2GRAY)
        
        # Apply histogram equalization to improve detection
        gray = cv2.equalizeHist(gray)
        
        # Fast detection on smaller frame - more sensitive settings
        faces = self.face_cascade.detectMultiScale(
            gray,
            scaleFactor=1.1,
            minNeighbors=3,
            minSize=(20, 20),
            maxSize=(300, 300),
            flags=cv2.CASCADE_SCALE_IMAGE
        )
        
        # Scale coordinates back to original frame
        face_list = []
        if len(faces) > 0:
            print(f"🔍 Detected {len(faces)} faces on {camera_id or 'unknown camera'}")
        
        for (x, y, w, h) in faces:
            # Scale back to original coordinates
            x = int(x / scale_factor)
            y = int(y / scale_factor) 
            w = int(w / scale_factor)
            h = int(h / scale_factor)
            
            # Add padding
            padding = 20
            x1 = max(0, x - padding)
            y1 = max(0, y - padding)
            x2 = min(frame.shape[1], x + w + padding)
            y2 = min(frame.shape[0], y + h + padding)
            
            face_dict = {
                'bbox': [x1, y1, x2, y2],
                'center': [x + w//2, y + h//2],
                'confidence': 1.0,
                'size': w * h
            }
            
            if camera_id:
                face_dict['camera_id'] = camera_id
                
            face_list.append(face_dict)
        
        return face_list
    
    def update_tracks(self, faces, frame=None, camera_id=None):
        """Update tracking - works for single or multi-camera"""
        current_time = time.time()
        
        if not faces:
            # Just increment disappeared counter
            for track_id in list(self.global_tracks.keys()):
                self.global_tracks[track_id]['disappeared'] += 1
                if self.global_tracks[track_id]['disappeared'] > self.max_disappeared:
                    if track_id in self.auto_registered_faces:
                        self.auto_registered_faces.remove(track_id)
                    del self.global_tracks[track_id]
            return list(self.global_tracks.values())
        
        # Create new tracks for first detections
        if not self.global_tracks:
            for face in faces:
                # Try to match with existing registered faces first
                matched_id = self.try_match_registered_face(face, frame)
                
                if matched_id:
                    track_id = matched_id
                else:
                    track_id = f"person_{self.next_id:03d}"
                    self.next_id += 1
                
                track_data = {
                    'id': track_id,
                    'center': face['center'],
                    'bbox': face['bbox'],
                    'disappeared': 0,
                    'last_seen': current_time,
                    'stability': 1,
                    'confidence': face['confidence'],
                    'name': self.faces_db.get(track_id, {}).get('name', track_id),
                    'is_registered': track_id in self.faces_db
                }
                
                if camera_id:
                    track_data['camera_id'] = camera_id
                    track_data['camera_history'] = [camera_id]
                
                self.global_tracks[track_id] = track_data
            return list(self.global_tracks.values())
        
        # Match faces to existing tracks
        used_tracks = set()
        
        for face in faces:
            best_track = None
            min_distance = float('inf')
            
            # First try to match with existing tracks
            for track_id, track in self.global_tracks.items():
                if track_id in used_tracks:
                    continue
                
                # Calculate distance (considering different cameras if multi-camera)
                distance = self.calculate_face_distance(face, track)
                
                if distance < min_distance and distance < self.max_distance:
                    min_distance = distance
                    best_track = track_id
            
            if best_track:
                # Update existing track with smoothing
                track = self.global_tracks[best_track]
                alpha = 0.3  # Less aggressive smoothing
                
                track['center'] = [
                    int(alpha * face['center'][0] + (1-alpha) * track['center'][0]),
                    int(alpha * face['center'][1] + (1-alpha) * track['center'][1])
                ]
                track['bbox'] = face['bbox']
                track['disappeared'] = 0
                track['last_seen'] = current_time
                track['stability'] = min(50, track['stability'] + 1)
                track['confidence'] = face['confidence']
                
                # Update camera info if multi-camera
                if camera_id:
                    if track.get('camera_id') != camera_id:
                        print(f"🔄 Face {track_id} moved to {camera_id}")
                    track['camera_id'] = camera_id
                    if 'camera_history' not in track:
                        track['camera_history'] = []
                    if camera_id not in track['camera_history']:
                        track['camera_history'].append(camera_id)
                
                used_tracks.add(best_track)
            else:
                # No existing track found - try to match with registered faces
                matched_id = None
                if self.use_embeddings and frame is not None:
                    matched_id = self.try_match_with_embeddings(face, frame)
                
                if not matched_id:
                    matched_id = self.try_match_registered_face(face, frame)
                
                if matched_id and matched_id not in self.global_tracks:
                    # Reuse the registered face ID
                    track_id = matched_id
                else:
                    # Create completely new track
                    track_id = f"person_{self.next_id:03d}"
                    self.next_id += 1
                
                track_data = {
                    'id': track_id,
                    'center': face['center'],
                    'bbox': face['bbox'],
                    'disappeared': 0,
                    'last_seen': current_time,
                    'stability': 1,
                    'confidence': face['confidence'],
                    'name': self.faces_db.get(track_id, {}).get('name', track_id),
                    'is_registered': track_id in self.faces_db
                }
                
                if camera_id:
                    track_data['camera_id'] = camera_id
                    track_data['camera_history'] = [camera_id]
                
                self.global_tracks[track_id] = track_data
        
        # Update disappeared counter for unused tracks
        for track_id in self.global_tracks:
            if track_id not in used_tracks:
                self.global_tracks[track_id]['disappeared'] += 1
        
        # Auto-register stable faces
        for track_id, track in self.global_tracks.items():
            if (not track['is_registered'] and 
                track['stability'] >= self.auto_register_threshold and 
                track['disappeared'] == 0 and
                track_id not in self.auto_registered_faces):
                
                # Find the frame to use for registration
                reg_frame = frame
                if self.is_multi_camera and track.get('camera_id'):
                    reg_frame = self.camera_frames.get(track['camera_id'], frame)
                
                if reg_frame is not None:
                    temp_name = f"temp_{track_id}"
                    camera_info = f" (seen on {len(track.get('camera_history', []))} cameras)" if self.is_multi_camera else ""
                    print(f"🤖 Auto-registering stable face: {temp_name}{camera_info}")
                    self.auto_registered_faces.add(track_id)
                    
                    if self.register_face(reg_frame, track['bbox'], track_id, temp_name):
                        track['is_registered'] = True
                        track['name'] = temp_name
        
        # Remove old tracks
        for track_id in list(self.global_tracks.keys()):
            max_disappeared = self.max_disappeared * 3 if self.global_tracks[track_id]['is_registered'] else self.max_disappeared
            if self.global_tracks[track_id]['disappeared'] > max_disappeared:
                if track_id in self.auto_registered_faces:
                    self.auto_registered_faces.remove(track_id)
                del self.global_tracks[track_id]
        
        return list(self.global_tracks.values())
    
    def calculate_face_distance(self, face, track):
        """Calculate distance between face and track"""
        dx = face['center'][0] - track['center'][0]
        dy = face['center'][1] - track['center'][1]
        base_distance = np.sqrt(dx*dx + dy*dy)
        
        # If multi-camera and different cameras, allow slightly larger distances
        if (self.is_multi_camera and 
            face.get('camera_id') and track.get('camera_id') and
            face['camera_id'] != track['camera_id']):
            return base_distance * 1.5  # Penalty for camera change
        
        return base_distance
    
    def try_match_registered_face(self, face, frame):
        """Try to match a detected face with registered faces"""
        if not self.faces_db:
            return None
        
        face_center = face['center']
        face_size = face['size']
        
        best_match = None
        best_score = 0
        
        for face_id, face_data in self.faces_db.items():
            if 'center' in face_data and 'size' in face_data:
                stored_center = face_data['center']
                stored_size = face_data['size']
            elif 'bbox' in face_data:
                stored_bbox = face_data['bbox']
                stored_center = [(stored_bbox[0] + stored_bbox[2]) // 2, (stored_bbox[1] + stored_bbox[3]) // 2]
                stored_size = (stored_bbox[2] - stored_bbox[0]) * (stored_bbox[3] - stored_bbox[1])
            else:
                continue
            
            # Calculate distance
            dx = face_center[0] - stored_center[0]
            dy = face_center[1] - stored_center[1]
            distance = np.sqrt(dx*dx + dy*dy)
            
            # Calculate size similarity
            size_ratio = min(face_size, stored_size) / max(face_size, stored_size) if max(face_size, stored_size) > 0 else 0
            
            # Create a matching score (higher is better)
            distance_score = max(0, 1 - (distance / (self.max_distance * 2)))
            size_score = size_ratio
            
            # Combined score
            combined_score = (distance_score * 0.7 + size_score * 0.3)
            
            if combined_score > best_score and combined_score > 0.4:
                best_score = combined_score
                best_match = face_id
        
        if best_match:
            self.faces_db[best_match]['center'] = face_center
            self.faces_db[best_match]['size'] = face_size
            self.faces_db[best_match]['last_seen'] = datetime.now().isoformat()
            
        return best_match
    
    def try_match_with_embeddings(self, face, frame):
        """Try to match face using embeddings (more accurate)"""
        if not self.use_embeddings or not self.model_manager:
            return None
        
        # Extract face region
        x1, y1, x2, y2 = face['bbox']
        face_img = frame[y1:y2, x1:x2]
        
        if face_img.size == 0:
            return None
        
        # Extract embedding for current face
        current_embedding = self.model_manager.extract_embedding(face_img)
        if current_embedding is None:
            return None
        
        # Compare with all registered faces
        best_match = None
        best_distance = float('inf')
        
        for face_id, face_data in self.faces_db.items():
            if 'embedding' in face_data:
                distance = self.model_manager.compare_faces(current_embedding, face_data['embedding'])
                
                if distance < best_distance and distance < self.embedding_threshold:
                    best_distance = distance
                    best_match = face_id
        
        if best_match:
            self.faces_db[best_match]['embedding'] = current_embedding
            self.faces_db[best_match]['last_seen'] = datetime.now().isoformat()
            print(f"🎯 Matched face with embedding: {best_match} (distance: {best_distance:.3f})")
        
        return best_match
    
    def start_camera_feed(self, camera_name, camera_source):
        """Start a single camera feed"""
        print(f"🎥 Starting {camera_name}: {camera_source}")
        
        # Initialize camera
        if camera_source.isdigit():
            cap = cv2.VideoCapture(int(camera_source))
        else:
            cap = cv2.VideoCapture(camera_source)
        
        # Set camera properties
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 480)
        cap.set(cv2.CAP_PROP_FPS, 15 if self.is_multi_camera else 30)
        cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
        
        if not cap.isOpened():
            print(f"❌ Failed to open {camera_name}: {camera_source}")
            return
        
        self.camera_feeds[camera_name] = cap
        self.camera_fps[camera_name] = 0
        
        frame_count = 0
        last_fps_time = time.time()
        
        print(f"✅ {camera_name} started successfully")
        
        failed_reads = 0
        max_failed_reads = 10
        
        while self.running:
            ret, frame = cap.read()
            if not ret:
                failed_reads += 1
                if failed_reads <= 5:  # Only show first few errors
                    print(f"⚠️ Failed to read from {camera_name} (attempt {failed_reads})")
                elif failed_reads == max_failed_reads:
                    print(f"❌ Too many failures for {camera_name}, stopping...")
                    break
                time.sleep(1)
                continue
            
            failed_reads = 0  # Reset counter on successful read
            
            # Store current frame
            self.camera_frames[camera_name] = frame.copy()
            
            # Calculate FPS
            frame_count += 1
            current_time = time.time()
            if current_time - last_fps_time >= 1.0:
                self.camera_fps[camera_name] = frame_count / (current_time - last_fps_time)
                frame_count = 0
                last_fps_time = current_time
            
            time.sleep(0.03)  # ~30 FPS max per camera
        
        cap.release()
        print(f"🛑 {camera_name} stopped")
    
    def start_all_cameras(self):
        """Start all configured cameras"""
        self.running = True
        
        for camera_name, camera_source in self.camera_config.items():
            thread = threading.Thread(
                target=self.start_camera_feed, 
                args=(camera_name, camera_source),
                daemon=True
            )
            self.camera_threads[camera_name] = thread
            thread.start()
            time.sleep(0.5)  # Stagger camera starts
        
        print(f"🚀 Started {len(self.camera_config)} camera feeds")
    
    def stop_all_cameras(self):
        """Stop all cameras"""
        self.running = False
        
        # Wait for threads to finish
        for thread in self.camera_threads.values():
            thread.join(timeout=2)
        
        # Release any remaining captures
        for cap in self.camera_feeds.values():
            if cap:
                cap.release()
        
        print("⏹️ All cameras stopped")
    
    def process_all_frames(self):
        """Process faces from all camera frames"""
        if not self.camera_frames:
            print("⚠️ No camera frames available for processing")
            return {}
        
        all_faces = []
        
        # Detect faces in all camera frames (use snapshot to avoid runtime errors)
        camera_frames_snapshot = dict(self.camera_frames)
        for camera_name, frame in camera_frames_snapshot.items():
            if frame is not None:
                faces = self.detect_faces(frame, camera_name)
                all_faces.extend(faces)
            else:
                print(f"⚠️ Frame from {camera_name} is None")
        
        # Update global tracking with all faces
        if self.is_multi_camera:
            # Multi-camera: process faces by camera to maintain camera association
            for camera_name, frame in camera_frames_snapshot.items():
                if frame is not None:
                    camera_faces = [f for f in all_faces if f.get('camera_id') == camera_name]
                    if camera_faces:
                        self.update_tracks(camera_faces, frame, camera_name)
        else:
            # Single camera: use first camera's frame
            first_camera = list(camera_frames_snapshot.keys())[0]
            first_frame = camera_frames_snapshot[first_camera]
            camera_faces = self.detect_faces(first_frame)
            self.update_tracks(camera_faces, first_frame, first_camera)
        
        return all_faces
    
    def get_display_frames(self):
        """Get display frames with overlays for all cameras"""
        display_frames = {}
        
        # Use snapshot to avoid runtime errors
        camera_frames_snapshot = dict(self.camera_frames)
        for camera_name, frame in camera_frames_snapshot.items():
            if frame is not None:
                display_frame = frame.copy()
                self.draw_overlay(display_frame, camera_name)
                display_frames[camera_name] = display_frame
        
        return display_frames
    
    def draw_overlay(self, frame, camera_name):
        """Draw overlay on frame"""
        h, w = frame.shape[:2]
        
        # Draw faces for this camera
        for track in self.global_tracks.values():
            should_draw = False
            
            if self.is_multi_camera:
                # Multi-camera: only draw faces currently on this camera
                should_draw = (track.get('camera_id') == camera_name and track['disappeared'] == 0)
            else:
                # Single camera: draw all active faces
                should_draw = (track['disappeared'] == 0)
            
            if should_draw:
                x1, y1, x2, y2 = [int(coord) for coord in track['bbox']]
                name = track['name']
                is_registered = track['is_registered']
                stability = track['stability']
                
                # Ensure coordinates are within frame bounds
                h, w = frame.shape[:2]
                x1 = max(0, min(x1, w-1))
                y1 = max(0, min(y1, h-1))
                x2 = max(0, min(x2, w-1))
                y2 = max(0, min(y2, h-1))
                
                # Color scheme: Red and Black theme
                if is_registered:
                    color = (0, 0, 255)  # Red for registered (BGR format)
                    thickness = 4  # Thicker for better visibility
                elif stability > 10:
                    color = (0, 140, 255)  # Orange for stable
                    thickness = 3
                else:
                    color = (100, 100, 100)  # Gray for unstable
                    thickness = 2
                
                # Main box with better visibility
                cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness)
                # Add corner markers for better visibility
                corner_size = 15
                cv2.line(frame, (x1, y1), (x1 + corner_size, y1), color, thickness + 1)
                cv2.line(frame, (x1, y1), (x1, y1 + corner_size), color, thickness + 1)
                cv2.line(frame, (x2, y1), (x2 - corner_size, y1), color, thickness + 1)
                cv2.line(frame, (x2, y1), (x2, y1 + corner_size), color, thickness + 1)
                cv2.line(frame, (x1, y2), (x1 + corner_size, y2), color, thickness + 1)
                cv2.line(frame, (x1, y2), (x1, y2 - corner_size), color, thickness + 1)
                cv2.line(frame, (x2, y2), (x2 - corner_size, y2), color, thickness + 1)
                cv2.line(frame, (x2, y2), (x2, y2 - corner_size), color, thickness + 1)
                
                # Label with better visibility
                if self.is_multi_camera:
                    camera_count = len(track.get('camera_history', []))
                    label = f"{name} (C:{camera_count})"
                else:
                    label = name
                
                # Add stability indicator
                status_icon = "●" if is_registered else "○"
                full_label = f"{status_icon} {label} ({stability})"
                
                font = cv2.FONT_HERSHEY_SIMPLEX
                scale = 0.6  # Slightly larger for better readability
                text_thickness = 2
                
                (text_w, text_h), _ = cv2.getTextSize(full_label, font, scale, text_thickness)
                
                # Enhanced background for text visibility
                padding = 8
                bg_x1 = max(0, x1 - 2)
                bg_y1 = max(0, y1 - text_h - padding * 2)
                bg_x2 = min(w, x1 + text_w + padding * 2)
                bg_y2 = y1
                
                # Semi-transparent black background
                overlay = frame.copy()
                cv2.rectangle(overlay, (bg_x1, bg_y1), (bg_x2, bg_y2), (0, 0, 0), -1)
                cv2.addWeighted(overlay, 0.8, frame, 0.2, 0, frame)
                
                # White outline for text visibility
                text_x = x1 + padding
                text_y = y1 - padding
                cv2.putText(frame, full_label, (text_x, text_y), font, scale, (255, 255, 255), text_thickness + 1)
                cv2.putText(frame, full_label, (text_x, text_y), font, scale, color, text_thickness)
        
        # Enhanced header with better visibility
        header_height = 70
        overlay = frame.copy()
        cv2.rectangle(overlay, (0, 0), (w, header_height), (0, 0, 0), -1)
        cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
        
        # Camera name and FPS with better formatting
        fps = self.camera_fps.get(camera_name, 0)
        active_faces = len([t for t in self.global_tracks.values() if t['disappeared'] == 0])
        registered_faces = len([t for t in self.global_tracks.values() if t.get('is_registered', False) and t['disappeared'] == 0])
        
        if self.is_multi_camera:
            title = f"{camera_name.replace('_', ' ')} | FPS: {fps:.1f}"
            stats = f"Faces: {active_faces} | Registered: {registered_faces}"
        else:
            title = f"Face Recognition | FPS: {fps:.1f}"
            stats = f"Active: {active_faces} | Registered: {registered_faces}"
        
        # Title with outline for better visibility
        cv2.putText(frame, title, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 3)
        cv2.putText(frame, title, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
        
        # Stats
        cv2.putText(frame, stats, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
        cv2.putText(frame, stats, (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 140, 255), 1)
        
        # Timestamp indicator
        timestamp = datetime.now().strftime("%H:%M:%S")
        cv2.putText(frame, timestamp, (w - 100, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (100, 100, 100), 1)
    
    def get_stable_faces_for_ui(self):
        """Get faces stable enough for UI interaction"""
        current_time = time.time()
        
        # Only update stable faces every 2 seconds to prevent flickering
        if current_time - self.last_ui_update < 2.0:
            return list(self.stable_faces.values())
        
        self.last_ui_update = current_time
        new_stable_faces = {}
        
        for track_id, track in self.global_tracks.items():
            if track['stability'] >= self.min_stability_for_ui and track['disappeared'] == 0:
                stable_face = {
                    'id': track_id,
                    'bbox': track['bbox'],
                    'confidence': track['confidence'],
                    'name': track['name'],
                    'is_registered': track['is_registered'],
                    'stability': track['stability']
                }
                
                if self.is_multi_camera:
                    stable_face['camera_id'] = track.get('camera_id', 'unknown')
                    stable_face['camera_count'] = len(track.get('camera_history', []))
                
                new_stable_faces[track_id] = stable_face
        
        self.stable_faces = new_stable_faces
        return list(self.stable_faces.values())
    
    def switch_model(self, model_name):
        """Switch to a different face recognition model"""
        if not FACE_MODELS_AVAILABLE:
            print("❌ Face recognition models not available")
            return False
            
        if self.model_manager.load_model(model_name):
            self.current_model = model_name
            self.use_embeddings = True
            print(f"✅ Switched to {model_name}")
            return True
        else:
            print(f"❌ Failed to switch to {model_name}")
            return False
    
    def get_available_models(self):
        """Get list of available models"""
        if not FACE_MODELS_AVAILABLE:
            return []
        return self.model_manager.list_models()
    
    def delete_face(self, face_id):
        """Delete a registered face"""
        if face_id in self.faces_db:
            # Remove image file
            face_data = self.faces_db[face_id]
            if 'image_path' in face_data and os.path.exists(face_data['image_path']):
                try:
                    os.remove(face_data['image_path'])
                    print(f"🗑️ Deleted image: {face_data['image_path']}")
                except Exception as e:
                    print(f"⚠️ Failed to delete image: {e}")
            
            # Remove from database
            del self.faces_db[face_id]
            
            # Remove from active tracks
            if face_id in self.global_tracks:
                del self.global_tracks[face_id]
            
            # Save updated database
            self.save_database()
            print(f"✅ Deleted face: {face_id}")
            return True
        else:
            print(f"❌ Face not found: {face_id}")
            return False
    
    def register_face(self, frame, bbox, face_id, name=None):
        """Register a face with optional embedding extraction"""
        face_path = self.save_face_image(frame, bbox, face_id)
        
        if face_path:
            # Calculate face characteristics for better matching
            x1, y1, x2, y2 = bbox
            center = [(x1 + x2) // 2, (y1 + y2) // 2]
            size = (x2 - x1) * (y2 - y1)
            
            face_data = {
                'name': name or face_id,
                'image_path': face_path,
                'registered_at': datetime.now().isoformat(),
                'bbox': [int(x) for x in bbox],
                'center': center,
                'size': size,
                'last_seen': datetime.now().isoformat()
            }
            
            # Extract embedding if using face recognition models
            if self.use_embeddings and self.model_manager:
                face_img = frame[y1:y2, x1:x2]
                embedding = self.model_manager.extract_embedding(face_img)
                if embedding is not None:
                    face_data['embedding'] = embedding.tolist()  # Convert to list for JSON serialization
                    print(f"🧠 Extracted {self.current_model} embedding for {name or face_id}")
            
            self.faces_db[face_id] = face_data
            
            # Update track if it exists
            if face_id in self.global_tracks:
                self.global_tracks[face_id]['name'] = name or face_id
                self.global_tracks[face_id]['is_registered'] = True
            
            self.save_database()
            model_info = f" ({self.current_model})" if self.use_embeddings else ""
            print(f"✅ Registered: {name or face_id} ({face_id}){model_info}")
            return True
        return False
    
    def save_face_image(self, frame, bbox, face_id):
        """Save face image"""
        x1, y1, x2, y2 = bbox
        face_img = frame[y1:y2, x1:x2]
        
        if face_img.size > 0 and face_img.shape[0] > 30 and face_img.shape[1] > 30:
            face_img = cv2.resize(face_img, (150, 150))
            face_path = f'faces/{face_id}.jpg'
            cv2.imwrite(face_path, face_img)
            return face_path
        return None
    
    def rename_face(self, face_id, new_name):
        """Rename a face"""
        if face_id in self.faces_db:
            old_name = self.faces_db[face_id]['name']
            self.faces_db[face_id]['name'] = new_name
            
            # Update track if it exists
            if face_id in self.global_tracks:
                self.global_tracks[face_id]['name'] = new_name
            
            self.save_database()
            print(f"✏️ Renamed: {old_name} → {new_name}")
            return True
        return False
    
    def load_database(self):
        """Load face database"""
        if os.path.exists('faces_db.json'):
            try:
                with open('faces_db.json', 'r') as f:
                    loaded_db = json.load(f)
                
                # Convert embedding lists back to numpy arrays
                self.faces_db = {}
                for face_id, face_data in loaded_db.items():
                    clean_data = face_data.copy()
                    if 'embedding' in clean_data and isinstance(clean_data['embedding'], list):
                        clean_data['embedding'] = np.array(clean_data['embedding'])
                    self.faces_db[face_id] = clean_data
            except Exception as e:
                print(f"Error loading database: {e}")
                self.faces_db = {}
        else:
            self.faces_db = {}
    
    def save_database(self):
        """Save face database"""
        try:
            # Convert numpy arrays to lists for JSON serialization
            clean_db = {}
            for face_id, face_data in self.faces_db.items():
                clean_data = face_data.copy()
                if 'embedding' in clean_data and hasattr(clean_data['embedding'], 'tolist'):
                    clean_data['embedding'] = clean_data['embedding'].tolist()
                clean_db[face_id] = clean_data
            
            with open('faces_db.json', 'w') as f:
                json.dump(clean_db, f, indent=2)
        except Exception as e:
            print(f"Error saving database: {e}")
    
    def get_faces_for_api(self):
        """Get faces data cleaned for JSON serialization"""
        clean_faces = {}
        for face_id, face_data in self.faces_db.items():
            clean_data = {}
            for key, value in face_data.items():
                if key == 'embedding':
                    # Skip embedding data for API responses (too large)
                    continue
                elif hasattr(value, 'tolist'):
                    # Convert numpy arrays to lists
                    clean_data[key] = value.tolist()
                else:
                    clean_data[key] = value
            clean_faces[face_id] = clean_data
        return clean_faces


class OptimizedFaceApp:
    def __init__(self):
        self.tracker = StableFaceTracker()
        self.running = False
        self.display_frames = {}
        
        # Processing control
        process_rate = 3 if self.tracker.is_multi_camera else 2
        self.process_every_n_frames = process_rate
        self.frame_counter = 0
        
    def run_background_processing(self):
        """Run background face processing with web display"""
        self.tracker.start_all_cameras()
        self.running = True
        
        mode_desc = "Multi-Camera" if self.tracker.is_multi_camera else "Single Camera"
        print(f"\n🎯 {mode_desc} Face Recognition System - Web Interface Mode")
        print("📱 Web interface: http://localhost:8080")
        print("\n💡 Features:")
        if self.tracker.is_multi_camera:
            print("  • Multiple camera feeds (RTSP/USB/Webcam)")
            print("  • Global face tracking across all cameras")
            print("  • Cross-camera face consistency")
        else:
            print("  • Single camera feed")
            print("  • Stable face tracking")
        print("  • Multiple model support with 13+ models")
        print("  • Face registration and deletion via web interface")
        print("  • Model switching and benchmarking")
        print(f"\n📷 Active cameras: {len(self.tracker.camera_config)}")
        print("\n⏹️ Press Ctrl+C to stop")
        
        try:
            while self.running:
                self.frame_counter += 1
                
                # Process face detection and tracking
                if self.frame_counter % self.process_every_n_frames == 0:
                    faces = self.tracker.process_all_frames()
                    if self.frame_counter % 30 == 0:  # Log every 10 processing cycles
                        print(f"📊 Processing cycle {self.frame_counter//self.process_every_n_frames}: {len(faces)} faces detected, {len(self.tracker.global_tracks)} tracks")
                
                # Update display frames
                self.display_frames = self.tracker.get_display_frames()
                if self.frame_counter % 60 == 0:  # Log display frames occasionally
                    print(f"🖼️ Display frames updated: {len(self.display_frames)} cameras with frames")
                
                time.sleep(0.03)  # ~30 FPS for processing
        except KeyboardInterrupt:
            print("\n🛑 Stopping...")
        
        self.stop()
    
    def stop(self):
        """Stop the application"""
        self.running = False
        self.tracker.stop_all_cameras()


# Web Interface Handler
class WebHandler(SimpleHTTPRequestHandler):
    def __init__(self, *args, app=None, **kwargs):
        self.app = app
        super().__init__(*args, **kwargs)
    
    def log_message(self, format, *args):
        pass  # Suppress logging
    
    def do_GET(self):
        if self.path == '/' or self.path == '/index.html':
            self.serve_interface()
            
        elif self.path == '/api/faces':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            faces = self.app.tracker.get_faces_for_api()
            self.wfile.write(json.dumps(faces).encode())
            
        elif self.path == '/api/current_faces':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            
            # Get stable faces for UI (reduces flickering)
            stable_faces = self.app.tracker.get_stable_faces_for_ui()
            
            response = {
                'faces': stable_faces,
                'total_registered': len(self.app.tracker.faces_db),
                'active_tracks': len(self.app.tracker.global_tracks),
                'current_model': self.app.tracker.current_model if self.app.tracker.use_embeddings else 'basic',
                'use_embeddings': self.app.tracker.use_embeddings,
                'active_cameras': len(self.app.tracker.camera_frames),
                'is_multi_camera': self.app.tracker.is_multi_camera
            }
            self.wfile.write(json.dumps(response).encode())
            
        elif self.path == '/api/models':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            
            models = self.app.tracker.get_available_models()
            response = {
                'available_models': models,
                'current_model': self.app.tracker.current_model,
                'use_embeddings': self.app.tracker.use_embeddings
            }
            self.wfile.write(json.dumps(response).encode())
            
        elif self.path == '/api/video_frame':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            
            # Get single camera frame for single-camera mode
            frame_data = None
            if self.app.display_frames:
                first_camera = list(self.app.display_frames.keys())[0]
                frame = self.app.display_frames.get(first_camera)
                if frame is not None:
                    _, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
                    frame_data = base64.b64encode(buffer).decode('utf-8')
            
            response = {'frame': frame_data}
            self.wfile.write(json.dumps(response).encode())
            
        elif self.path == '/api/camera_feeds':
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            
            # Get all camera frames as base64
            camera_data = {}
            for camera_name, frame in self.app.display_frames.items():
                if frame is not None:
                    _, buffer = cv2.imencode('.jpg', frame, [cv2.IMWRITE_JPEG_QUALITY, 70])
                    frame_data = base64.b64encode(buffer).decode('utf-8')
                    fps = self.app.tracker.camera_fps.get(camera_name, 0)
                    
                    camera_data[camera_name] = {
                        'frame': frame_data,
                        'fps': fps,
                        'timestamp': time.time()
                    }
            
            response = {
                'cameras': camera_data,
                'total_cameras': len(self.app.tracker.camera_config),
                'is_multi_camera': self.app.tracker.is_multi_camera
            }
            self.wfile.write(json.dumps(response).encode())
            
        else:
            # Serve static files (CSS, JS, images)
            super().do_GET()
    
    def do_POST(self):
        content_length = int(self.headers['Content-Length'])
        post_data = self.rfile.read(content_length)
        data = json.loads(post_data.decode())
        
        success = False
        
        if self.path == '/api/rename_face':
            face_id = data.get('face_id')
            new_name = data.get('new_name')
            success = self.app.tracker.rename_face(face_id, new_name)
            
        elif self.path == '/api/delete_face':
            face_id = data.get('face_id')
            print(f"🗑️ Delete request for face_id: {face_id}")
            success = self.app.tracker.delete_face(face_id)
            print(f"🗑️ Delete result: {success}")
            
        elif self.path == '/api/switch_model':
            model_name = data.get('model_name')
            success = self.app.tracker.switch_model(model_name)
        
        self.send_response(200)
        self.send_header('Content-type', 'application/json')
        self.end_headers()
        self.wfile.write(json.dumps({'success': success}).encode())
    
    def serve_interface(self):
        """Serve the appropriate interface based on camera mode"""
        self.send_response(200)
        self.send_header('Content-type', 'text/html')
        self.end_headers()
        
        if self.app.tracker.is_multi_camera:
            self.serve_multi_camera_html()
        else:
            self.serve_single_camera_html()
    
    def serve_single_camera_html(self):
        """Serve single camera interface"""
        with open('index.html', 'r') as f:
            html_content = f.read()
        self.wfile.write(html_content.encode())
    
    def serve_multi_camera_html(self):
        """Serve multi-camera interface"""
        with open('multi-camera.html', 'r') as f:
            html_content = f.read()
        self.wfile.write(html_content.encode())


def run_face_recognition():
    """Main function"""
    app = OptimizedFaceApp()
    
    # Graceful shutdown
    def signal_handler(sig, frame):
        print("\n🛑 Shutting down...")
        app.stop()
        sys.exit(0)
    
    signal.signal(signal.SIGINT, signal_handler)
    
    # Start web server
    def start_web_server():
        def create_handler(*args, **kwargs):
            return WebHandler(*args, app=app, **kwargs)
        
        server = HTTPServer(('localhost', 8080), create_handler)
        print("🌐 Web interface: http://localhost:8080")
        
        try:
            server.serve_forever()
        except:
            pass
    
    web_thread = threading.Thread(target=start_web_server, daemon=True)
    web_thread.start()
    
    # Open browser
    def open_browser():
        time.sleep(1)
        try:
            webbrowser.open('http://localhost:8080')
        except:
            pass
    
    threading.Thread(target=open_browser, daemon=True).start()
    
    # Run background processing
    app.run_background_processing()


if __name__ == "__main__":
    run_face_recognition()